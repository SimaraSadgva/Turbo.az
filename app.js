...
banSelect.innerHTML += Array.from(banSet).map(ban => `<option value="${ban}">${ban}</option>`).join('');
currencySelect.innerHTML += Array.from(currencySet).map(currency => `<option value="${currency}">${currency}</option>`).join('');
citySelect.innerHTML += Array.from(citySet).map(city => `<option value="${city}">${city}</option>`).join('');
modelSelect.innerHTML += Array.from(modelSet).map(model => `<option value="${model}">${model}</option>`).join('');
markaSelect.innerHTML += Array.from(brandSet).map(brand => `<option value="${brand}">${brand}</option>`).join('');
...
function show() {
    ...
    data
        .filter(item => {
            if (qiymet2.value != "") {
                if (item.price >= +qiymet1.value && item.price <= +qiymet2.value) {
                    return true;
                }
            } else {
                return true;
        ...
        .map(item => {
            ...
            <div onclick="showCarCard('${item.id}')" >
                <div class="img-car ">
                    <img src="${item.images[0]}" class="w-[230px] h-48 w-full object-cover" alt="Car Image"> 
                </div>
                <div class="info mb-[30px] bg-gray-100 pb-2 pl-2">
                    <h2 class="font-bold text-red-900 text-lg">${item.price} ${item.currency}</h2>
...
function showCarCard(id) {
    ...
    if (card) {
        main2.innerHTML = `
            <div class="cardtop text-[24px] text-left ">
            <button onclick="goBack()" class="back-button text-white rounded-[10px] text-center my-[20px] py-[3px] px-[5px] bg-gray-300 border border-red-700  hover:bg-red-800">Geri Qayıt</button>
                <h1 class="text-[30px] font-bold text-red-900">${card.brand}</h1>
                <p><b>Model:</b> ${card.model}</p>
                <p><b>Ban Növü:</b> ${card.banType}</p>
                <p><b>Qiymət:</b> ${card.price} ${card.currency}</p>
                <p><b>Odometr:</b> ${card.odometer} ${card.odometerUnit}</p>
                <p class="text-gray-600"> ${card.city} , ${card.year} , ${card.engine}L </p>
            </div>
            <div class="lg:max-w-lg lg:w-full md:w-1/2 w-5/6">
                <img class="object-cover object-center rounded" src="${card.images[0]}" alt="car image">    
            </div>
        `;
    }
}
